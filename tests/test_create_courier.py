...
class TestCourierCreate:
    ...
    def test_courier_can_be_created(self, courier_cleanup):
    ...
    def test_cannot_create_duplicate_courier(self, courier_cleanup):
        courier_data = generate_courier_data()
        ...
        with allure.step("Проверяем, что вернулся 409 и корректное сообщение"):
            ...
            # Добавляем в cleanup только после успешного теста
            courier_cleanup(courier_data)
       

    @allure.story("Create courier")
    @pytest.mark.parametrize("missing_field", MISSING_REQUIRED_FIELDS)
    def test_missing_required_field_returns_error(self, missing_field, courier_cleanup):
    ...
    def test_missing_first_name_is_allowed(self, courier_cleanup):
        payload = generate_courier_data()
